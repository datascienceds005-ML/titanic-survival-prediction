...
def generate_titanic_data(n_samples: int = 891, seed: int = 42) -> pd.DataFrame:
    """Generate realistic Titanic-like dataset."""
    np.random.seed(seed)
    n = n_samples
    ...
    age    = np.where(
        ...
        np.random.normal(29.7, 14.5, n).clip(0.17, 80)
    )
    sibsp  = np.random.choice([0,1,2,3,4,5,8], n, p=[0.68,0.23,0.05,0.02,0.01,0.005,0.005])
    parch  = np.random.choice([0,1,2,3,4,5,6], n, p=[0.76,0.13,0.09,0.005,0.003,0.001,0.001])
    fare   = np.where(
    ...
    embarked = np.random.choice(["S","C","Q",np.nan], n, p=[0.72,0.19,0.09,0.002])
    cabin    = np.where(np.random.random(n) < 0.77, np.nan,
                        np.random.choice(["A","B","C","D","E","F","G"], n))

    titles = np.where(sex=="male",
                      np.random.choice(["Mr","Dr","Rev","Master"],n,p=[0.94,0.02,0.02,0.02]),
                      np.random.choice(["Mrs","Miss","Ms","Dr"],n,p=[0.47,0.48,0.03,0.02]))

    names = [f"{titles[i]}_Passenger_{i}" for i in range(n)]
    ...
    surv_prob = np.where(np.isnan(age), surv_prob, surv_prob)
    survived = (np.random.random(n) < surv_prob.clip(0.02, 0.98)).astype(int)
